...
class WallpaperListViewModel extends BaseViewModel {
  ...
  final BehaviorSubject<WallpaperListState> _stateSubject = BehaviorSubject<WallpaperListState>.seeded(
  ...
  void _init() {
    final CancelableOperation<List<td.Background>> operation =
        _backgroundRepository.backgrounds.toCancelableOperation().onValue((List<td.Background> backgrounds) {
      _stateSubject.add(
  ...
  List<ITileModel> _mapTileModels(List<td.Background> backgrounds) => backgrounds
      .map((td.Background background) {
        ...
        switch (background.type.getConstructor()) {
          case td.BackgroundTypeWallpaper.constructor:
            {
              ...
              return BackgroundWallpaperTileModel(
                ...
                minithumbnail: background.document?.minithumbnail?.toMinithumbnail(),
              );
            }
          case td.BackgroundTypePattern.constructor:
            {
              final td.BackgroundTypePattern fill = background.type as td.BackgroundTypePattern;
              final td.Document document = background.document!;
          ...
          case td.BackgroundTypeFill.constructor:
            {
              final td.BackgroundTypeFill fill = background.type as td.BackgroundTypeFill;
